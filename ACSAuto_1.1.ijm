...
function getlineLength () {
	...
	*/
	...
	if (scaling == "Automatic") {
		...
		// delete unnecessary parts of image
		makePolygon(420,76,435,404,448,700,614,803,808,807,956,618,978,296,817,112);
		Roi.setStrokeColor("black");
		setBackgroundColor(0, 0, 0);
		run("Clear", "slice");
		n0 = nResults;
		print(n0);
		run("Ridge Detection", "line_width=1 high_contrast=128 low_contrast=70 extend_line show_ids displayresults add_to_manager method_for_overlap_resolution=NONE sigma=0.79 lower_threshold=9 upper_threshold=18 minimum_line_length=175 maximum=280");
		...
		//delete unnecessary parts of image
		makeRectangle(413, 64, 606, 757);
		Roi.setStrokeColor("black");
		setBackgroundColor(0, 0, 0);
		run("Clear", "slice");
		n0 = nResults;
		print(n0);
		run("Ridge Detection", "line_width=1 high_contrast=128 low_contrast=70 extend_line show_ids displayresults add_to_manager method_for_overlap_resolution=NONE sigma=0.79 lower_threshold=9 upper_threshold=18 minimum_line_length=200 maximum=500");
		...
		// deleting unnecessary parts of image
		makeRectangle(366, 67, 700, 700);
		Roi.setStrokeColor("black");
		setBackgroundColor(0, 0, 0);
		run("Clear", "slice");
		n0 = nResults;
		print(n0);
		run("Ridge Detection", "line_width=1 high_contrast=128 low_contrast=70 extend_line show_ids displayresults add_to_manager method_for_overlap_resolution=NONE sigma=0.79 lower_threshold=9 upper_threshold=20 minimum_line_length=350 maximum=700");
...
function circScanLine(x, y, xs, ys, count, nbeams, R, N, thresh, angle_start, angle_end) {
	...
	*/
 	...
 	//start und endwinkel sind 0 und 360 weil man ganz im Kreis will. x und y sind mittelpunkt vom Kreis, xs un ys werden verwendet für die gefunden outline points, count ist anzahl outline point die wir bis dahni gefunden haben und die wird upgedated. R bestimmt wie lange der strahl in pixel ist, N ist in wie viele schritte man vom  Mittlepunkt rausgeht. Bei N zu klein kann es sein das pixel geskpiit werden. R und N bestimmen die Schrittgrösse. 
	...
	*/
	...
	for (i = 0; i < nsteps; i++) {	
		...
		for (j = 0; j < N; j++) {
			...
			r = R * j / N;
			yi = round(i * (y1 - y0) / nsteps - sign * r) + y0; //wir gehen hier nicht im Kreis sondern vertikal und deswegen manipulieren wir das y. 
	...
	*/
	...
	for (i = 0; i < nsteps; i++) {	
		...
		for (j = 0; j < N; j++) {
			...
			// calculate current y-position
			r = R * j / N;
			xi = round(i * (x1 - x0) / nsteps + sign * r) + x0; //hier x weil horizontal
