...
#@ String (label = "Mode Name", choices = {"Rectus femoris", "Vastus lateralis", "Quad RF", "Quad VL", "Quadriceps", "Gastro MED"}, description="Select muscle(s) to be analysed on picture") muscle
...
#@ Boolean (label = "Sorting of coordinates", value = true, persist = false, describtion = "Whether to sort found outline coordinates to avoid overlap") sorting 
...
function getPreprocessingSettings(muscle) {
	...
	*/
	...
	if (muscle == "Gastro MED") {
		defaultMinLengthFac = 0.5;
		defaultTubenessSigma = 7;
...
function getlineLength () {
	...
	*/
	...
	if (scaling == "Automatic") {
		...
		n0 = nResults;
		run("Analyze Particles...", "size=650-Infinity show=Masks Clear");
		n1 = nResults;
		IJ.deleteRows(n0, n1);
...
function getAutoOutlineFinderStartingPoints(muscle) {
	...
	*/
	...
	if (muscle == "Quadriceps") {
 	...
 	if (muscle== "Gastro MED"){
 		starting_xy = newArray(6); 
 		starting_xy[0] = round(y + 0.25*height);
 		starting_xy[1] = round(x + 0.33*width); 
 		starting_xy[2] = round(y + 0.35*height);
 		starting_xy[3] = round(x + 0.7*width); 
 		starting_xy[4] = round(x + 0.6*height);
 		starting_xy[5] = round(x + 0.73*width);
 	}
...
function measureGastroMedArea(nbeams, nsteps, R, N, thresh) {
	/*
		Measures the area of the isolated vastus lateralis
	*/
...
function processImage(muscle) {
	...
	*/
	...
	} else if (muscle == "Gastro MED") {
		measureGastroMedArea(10, 10, 75, 30, 0);
	}
